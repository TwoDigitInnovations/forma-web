...
const SidePannel = ({ openTab, setOpenTab }) => {
  ...
  return (
    <>
      {/* <button
        className="lg:hidden fixed top-4 left-4 z-50 bg-blue-500 p-2 rounded"
        onClick={() => setOpenTab(true)}
      >
        <Menu className="text-white" />
      </button> */}
      <div
        className="hidden lg:flex fixed top-0 left-0 h-screen bg-[#1e293b] z-40 flex-col"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          ...
          {isProjectDetailsRoute && (
            <div
              className="m-3 bg-blue-500 flex items-center gap-3 justify-center rounded-lg cursor-pointer py-3"
      ...
      <div
        className={`fixed top-0 left-0 h-screen w-[260px] bg-[#1e293b] z-40 transform transition-transform duration-300 lg:hidden
            ...
            (item, i) =>
              item.access.includes(user?.role) && (
                <div
                  ...
                  ${
                    isActive(item.href)
                      ? "bg-blue-500 text-white rounded-xl m-2"
                      : "text-white hover:bg-white/10"
                  }`}
                >
